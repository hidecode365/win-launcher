...
<a id="no-color-transition-on-row-highlight"></a>

## 一覧行の選択ハイライト・ホバーに色のトランジションを使わない（issue 0030）

`MANAGE_TREE_ROW_BASE`（`sharedStyles.ts`）を含め、一覧行の選択ハイライト・ホバー背景には`transition-colors`を掛けない。一覧が再構成されて選択行が入れ替わった瞬間、`transition-colors`があると前の行の色が残ってフェードするため、「一瞬どこも選択されていない」ように見えてちらつく。

- 対象は通常検索の一覧行（`ResultList`）、お気に入り管理・メモ管理画面の全行（`MANAGE_TREE_ROW_BASE`経由）、クリップボード履歴行、パス貼り付けウィザードのフォルダ選択候補行、Web検索行、設定画面の検索フォルダ行ホバーなど、選択ハイライト・ホバーを持つ一覧行全般
- **行以外のトランジション（ボタン・行内アイコンのホバー、スプリッターのドラッグ、フォルダ開閉の回転、設定項目の有効/無効フェード等）は対象外で、従来どおり維持する。** ユーザー操作へ直接応答する演出であり、一覧の再構成に伴うちらつきの原因にならないため
- 新しく一覧行・選択可能な行を追加する場合、行の背景色・ハイライトには`transition`系クラスを付けない
