...
issue 0024でL1化したクリップボード履歴・最近使ったファイルのローカル絞り込み文字列（`clipboardEditFilterText`/`recentEditFilterText`）も同じ「設定画面往復をまたいで保持する」要件を満たすが、置き場所は`App.tsx`ではなく`useSearch.ts`内の`useState`である。`useSearch()`自体が`App.tsx`のトップレベルで無条件に呼ばれ常にマウントされ続けるフックのため、そこに状態を持たせるだけで同じ効果（コンポーネントの unmount/remount をまたいだ保持）が得られる。favorite/memoの「呼び出し元へ引き上げる」パターンと、clipboard/recentの「常時マウントされる既存フック内に置く」パターンは、どちらも同じ目的（設定往復での消失防止）を異なる置き場所で達成しているだけであり、新しいL1画面の絞り込み文字列を追加する際はその画面のデータが元々どのフックに属するかで置き場所を選べばよい（無理に統一する必要はない）。
...
理由：`search_files` は検索上限件数（`appSettings.searchMaxResults`。既定値・許容範囲はMG側要件定義01・05を正本とし、本書へ数値を重複記載しない）に達した時点で走査を打ち切る。フロントエンド側で受け取った結果からピン止め済み分を差し引く方式だと、ピン止め件数が多いほど「一覧に表示できる固有ファイル数」がその分減ってしまう。Rust側の候補生成ループ内（`continue` で早期スキップ）で除外すれば、上限の枠はピン止め対象外のファイルだけで満たされる。
