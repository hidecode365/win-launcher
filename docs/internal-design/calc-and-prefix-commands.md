...
- **「暗黙判定モードはファイル検索と共存させ、表示順序を固定する」という方針**は外部設計書 `external-design/04-platform-policies.md#implicit-mode-coexistence` へ移設した。実装上は、数式らしい入力であっても `search_files` とは排他にせず両方を独立して実行し、計算結果を別枠固定表示領域に描画する
