...
対象コード: `src/components/StatusFooter.tsx`（検索画面・パス貼り付けウィザードを1つに束ねた汎用フッター）、`src/components/FavoriteEditFooter.tsx`（お気に入り編集ビュー専用）、`src/components/MemoManageFooter.tsx`（メモ管理画面専用）、`src/components/ClipboardEditFooter.tsx`（クリップボード履歴画面専用）、`src/components/RecentEditFooter.tsx`（最近使ったファイル画面専用）、`src/components/OcrEditFooter.tsx`（OCR画面専用）、`src/components/SettingsPanel.tsx`（設定画面フッター）、`src/components/KeyHint.tsx`（キー操作ヒントのチップ表示。全フッター共通）、`src/components/FooterBar.tsx`（フッター外枠。左側にキー操作チップ群、右端にバージョン番号を表示するレイアウトを共通化）。

issue 0024（クリップボード履歴・最近使ったファイル・OCRのL1画面化）で、`StatusFooter.tsx`が持っていた`clipboardMode`分岐は撤去し、クリップボード履歴も他の2画面と同じく専用コンポーネントへ分離した（画面自体が検索画面から独立したL1になったため、`StatusFooter.tsx`側の条件分岐に留める理由が無くなった）。
...
| クリップボード履歴画面 | `ClipboardEditFooter.tsx`（専用コンポーネント） | （選択可能時）↑↓ 選択／Enter クリップボードにセット／Ctrl+D クリア／Esc 戻る |
| 最近使ったファイル画面 | `RecentEditFooter.tsx`（専用コンポーネント） | （選択可能時）↑↓ 選択／Enter 起動／Shift+Enter フォルダを開く／Ctrl+D クリア／Esc 戻る |
| OCR画面 | `OcrEditFooter.tsx`（専用コンポーネント） | Esc 戻る（Ctrl+Dは無効操作のためヒントを表示しない。「閉じる」「コピーして閉じる」は右ペインのActionButtonのみで提供し、フッターにEnter相当のヒントは持たせない） |
