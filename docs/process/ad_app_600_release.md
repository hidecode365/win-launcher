...
工程定義表全体は `WORKFLOW.md` を参照。本ファイルは 600_リリース 工程における AD（アプリ開発担当）自身の実施要領のみを扱う。旧 `.claude/skills/release-flow/SKILL.md`（自己起動 Skill）から移植した内容であり、内容は変更していない。

この工程は「PO → MG → AD」の経路で開始される運用に統一されている。POが AD に対して「リリースして」と自然文で直接指示し AD が自己起動する運用は行わない。そのため本ファイルには自己起動トリガー（Skill のフロントマター等）を持たせない。MG からの実行指示を受けて着手する。

## INPUT

- POのリリース判断（MG を経由して伝達される）
