...
この工程は「ひでさん → CA → CC」の経路で開始される運用に統一されている。ひでさんが CC に対して「リリースして」と自然文で直接指示し CC が自己起動する運用は行わない。そのため本ファイルには自己起動トリガー（Skill のフロントマター等）を持たせない。CA からの実行指示を受けて着手する。

## INPUT

- ひでさんのリリース判断（CA を経由して伝達される）
...
- ビルド前に、ユーザー自身が以下の環境変数を手元のシェルにセットしていること
  ...
  - **これらの値は CC が代わりに入力・保持・出力してはならない。** 未設定の場合はユーザーに設定を依頼し、CC 自身は値を尋ねない・推測しない
