...
この工程は「PO → CA → CC」の経路で開始される運用に統一されている。POが CC に対して「リリースして」と自然文で直接指示し CC が自己起動する運用は行わない。そのため本ファイルには自己起動トリガー（Skill のフロントマター等）を持たせない。CA からの実行指示を受けて着手する。

## INPUT

- POのリリース判断（CA を経由して伝達される）
