    ...
    <div className="relative flex h-screen flex-col overflow-hidden rounded-2xl border border-white/20 bg-white/90 shadow-2xl backdrop-blur-xl">
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto">
        {visibleRows.map((row, index) => {
          ...
          if (row.kind === "root") {
            return <Fragment key={node.id}><div role="button" data-index={index} className={rowClass} style={{ paddingLeft: "1rem" }} {...commonEvents}><span className="flex-1 truncate font-medium">メモ</span>{renderActionIcons(row, selected)}</div>{creatingAnchorId === node.id && creating === "folder" && <CreateFolderInlineRow depth={0} targetParentId={creatingParentId} onCreateFolder={createFolder} onFolderCreated={(id) => { setCreating(null); setCreatingAnchorId(null); selection.selectByKey(id, Date.now() + 1000); }} onCancel={() => { setCreating(null); setCreatingAnchorId(null); }} />}{creatingAnchorId === node.id && creating === "memo" && <MemoCreateRow depth={0} name={name} onNameChange={setName} onCreate={createMemo} onCancel={() => { setCreating(null); setCreatingAnchorId(null); }} />}</Fragment>;
          }
