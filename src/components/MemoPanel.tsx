...
export function MemoPanel({
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      } else if (event.ctrlKey && event.key.toLowerCase() === "e" && document) {
        event.preventDefault();
        event.stopImmediatePropagation();
  ...
  }, [document, onMoveSelection, saveWithFeedback]);
  ...
  return <ResizableSplitPane
    ...
    left={<div ref={listRef} className="h-full overflow-y-auto">
      {!hasMemo ? (
        ...
        <button key={node.id} data-index={index} type="button" onMouseEnter={() => onSelect(node.id, false)} onClick={() => { onSelect(node.id, false); if (!filterText) onToggleFolder(node.id, !node.collapsed); }} className={`flex w-full items-center py-2 pr-3 text-left text-xs font-medium ${node.id === selectedId ? "bg-blue-500 text-white" : "text-gray-500 hover:bg-gray-50"}`} style={{ paddingLeft: 12 + depth * 16 }}><FolderChevron collapsed={node.collapsed} /><svg className="ml-1.5 mr-2 h-4 w-4 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d={FOLDER_ICON_PATH} /></svg><span className="truncate">{node.name}</span></button> :
        <button key={node.id} data-index={index} type="button" onMouseEnter={() => onSelect(node.id, false)} onClick={() => onSelect(node.id, true)} className={`flex w-full items-center py-2 pr-3 text-left text-sm ${node.id === selectedId ? "bg-blue-500 text-white" : "text-gray-700 hover:bg-gray-100"}`} style={{ paddingLeft: 12 + depth * 16 }}><FileIcon className="mr-2 h-4 w-4 flex-shrink-0" /><span className="truncate">{node.name}</span></button>
      )}
    </div>}
