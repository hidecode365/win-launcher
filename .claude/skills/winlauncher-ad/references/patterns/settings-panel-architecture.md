...
- 設定画面のどの箇所にも縦ラインによる区切り（`border-l`）を使わない。階層構造は `SettingsIndent`、グループ見出しは `SettingsGroup` を使う。区切りは `gap` の広さ、または見出し＋横罫線で表現する。項目ラベル＋設定コントロールを横並びにする場合は `SettingsField` を使い、`justify-between` で両端に引き離さない。 → 詳細: [settings-panel-architecture.md](../../../../../docs/internal-design/settings-panel-architecture.md#indent-and-group)
