...
- 一覧行の選択ハイライト・ホバー背景には`transition-colors`等の色トランジションを使わない（一覧再構成時のちらつきの原因になる）。ボタン・行内アイコン・スプリッター・開閉アイコン等、行以外のトランジションは対象外で従来どおり使ってよい。 → 詳細: [shared-ui-system.md](../../../../../docs/internal-design/shared-ui-system.md#no-color-transition-on-row-highlight)
