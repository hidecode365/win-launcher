...
工程定義表全体は `WORKFLOW.md` を参照。本ファイルは 600_リリース 工程における AD（アプリ開発担当）自身の実施要領のみを扱う。

この工程は、リリース実行の明示的な指示を受けて着手する。指示の発信者がMG（通常の経路）かPO（MGが機能しない場合等の直接指示）かは問わない — 判断軸は「対象バージョン・実行内容が明確な指示を受けているか」である。git push・タグ push・`gh release create`・アセットアップロードはいずれも配布物を公開する高リスク操作のため、発信者を問わず着手前に対象バージョンとコマンド内容をユーザーに提示し、実行承認を得ること（詳細は「リリース手順（7ステップ）」節の該当ステップを参照）。

## INPUT

- POのリリース判断（MG経由、またはPOからの直接指示）
...
この動作確認は `npm run tauri dev` 起動後の自動 GUI テストの対象外（`AGENTS.md`「テストに関する制約」節）であり、ユーザーが手動で行う。
