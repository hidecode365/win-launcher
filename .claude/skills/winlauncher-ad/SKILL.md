...
この工程はMGからのhandoff経由で開始されるのが通常だが、MGが何らかの理由で機能しない場合等、POから直接明示的な指示を受けて着手してもよい。着手可否の判断軸は「発信者がMGかPOか」ではなく、対象・範囲が明確な実行指示を受領者（AD）自身が既存ルールと突き合わせて判断できるかどうかである。役割・対象アクター・対象リポジトリ／パス・操作範囲・必須成果物・承認条件が明確で、既存の`AGENTS.md`・このSkill・`WORKFLOW.md`と矛盾しなければ、発信者がMGでもPOでも追加のPO確認なしで通常手順どおり進める。

上記のいずれかが不明・過大、または既存ルールと矛盾する場合は、該当する編集・commit・push・公開操作を止める。疑義が指示の一部にしか及ばない場合は該当部分だけを止め、疑義があっても読み取り・影響整理・分析・handoffへの報告は継続してよい。標準工程（100〜600）の既定スコープに収まる指示は既定スコープの参照＋差分の簡略な指示で足り、000_運用管理のように標準工程に当てはまらない指示・既定スコープを拡張／縮小する指示・PO直接指示は指示境界の全項目を確認する。受領者チェックの詳細な手順は`ad_app_900_handoff-protocol.md`を正本とする。

特に配布物を公開する高リスク操作（600_リリース等）は、発信者を問わず着手前に対象バージョン・変更内容を提示し実行承認を得ること。
